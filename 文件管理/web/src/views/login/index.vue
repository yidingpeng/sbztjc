...
<script>
  ...
  export default defineComponent({
    ...
    setup() {
        ...
        state['formRef'].validate(async (valid) => {
          if (valid)
            try {
              ...
              localStorage['rememberMe'] = state.form.rememberMe
              localStorage['user'] = state.form.username
              if (state.form.rememberMe)
                localStorage['password'] = state.form.password
      ...
      onBeforeMount(() => {
        const { user, password, rememberMe } = localStorage
        state.form.rememberMe = rememberMe == 'true'
        if (user != null && user != '') {
          state.form.username = user
        }
        if (password != null && password != '') state.form.password = password
