  ...
  <div class="node-wrap">
    <div class="node-wrap-box" @click="show">
      <div class="title" style="background: #ff943e">
        <vab-icon class="icon" icon="user-fill" />
        <span>{{ nodeConfig.nodeName }}</span>
        <vab-icon class="close" icon="close-fill" @click.stop="delNode()" />
      </div>
      <div class="content">
      ...
      <template #title>
        <div class="node-wrap-drawer__title">
          <label v-if="!isEditTitle" @click="editTitle">
            {{ form.nodeName }}
            <vab-icon class="node-wrap-drawer__title-edit" icon="edit-2-fill" />
          </label>
          <el-input
            v-if="isEditTitle"
            ref="nodeTitle"
          ...
          <el-form label-position="top">
            <el-form-item label="审批人员类型">
              <el-select v-model="form.setType">
                ...
                <el-option label="发起人自己" :value="5" />
                <el-option label="连续多级主管" :value="7" />
              </el-select>
            </el-form-item>
