...
{
    public class DAL_DRoomFaultTime : IDAL_DRoomFaultTime

    {
        public int addFaultTime(DRoom_FaultTimeModel model)
        {
            ...
            string sql = @"insert into droom_FaultTime(deviceName,roomName,onOrOff,alarmTime) 
        ...
        public List<DRoom_FaultTimeModel> GetDevice_faultTimesdesc(string deviceName, string roomName)
        {
            ...
            List<MySqlParameter> pList = new List<MySqlParameter>();
            string sql = @"select * from droom_FaultTime where  roomName like @roomName   order by id desc limit 1";
            pList.Add(new MySqlParameter("@roomName", '%' + roomName + '%'));
            //pList.Add(new MySqlParameter("@deviceName", deviceName));

            List<DRoom_FaultTimeModel> list = db.ExecuteList<DRoom_FaultTimeModel>(sql, pList.ToArray());
            return list;
        }
        ...
        public List<DRoom_FaultTimeModel> GetOneDRoom_FaultTimeList(string roomName)
        {
            ...
            List<MySqlParameter> pList = new List<MySqlParameter>();
            string sql = @"select * from droom_FaultTime where  roomName like @roomName   order by id desc";
            pList.Add(new MySqlParameter("@roomName", '%' + roomName + '%'));
            //pList.Add(new MySqlParameter("@deviceName", deviceName));
