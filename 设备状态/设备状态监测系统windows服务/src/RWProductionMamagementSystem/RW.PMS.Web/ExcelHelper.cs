...
namespace RW.PMS.Web
{
    public class ExcelHelper
    {
        ...
        /// <param name="filePath"></param>
        /// <returns></returns>
        public static DataTable GetExcelDataToTable(string filePath)
        {
            DataTable table = new DataTable();
            try
            {
                using (FileStream fileStream = new FileStream(filePath, FileMode.Open, FileAccess.Read))
                {
                    ...
                    if (fileExt == ".xls")
                    {
                        table = ExcelToTableForXLS(fileStream);
                    }
                    else if (fileExt == ".xlsx")
                    {
                        table = ExcelToTableForXLSX(fileStream);
                    }
                }
            }
            catch (Exception ex)
        ...
        /// <summary>
        /// 读取数据到XLXS文件（excel2007）
        /// </summary>
        /// <param name="fs"></param>
        /// <returns></returns>
        public static DataTable ExcelToTableForXLSX(FileStream fs)
        {
            DataTable dt = new DataTable();
            ...
            if (xssfworkbook != null)
            {
                ...
                if (sheet != null)
                {
                    ...
                    //取出列名
                    for (int i = firstRow.LastCellNum; i < cellCount; i++)
                    {
                        XSSFCell cell = (XSSFCell)firstRow.GetCell(i);
                        if (cell != null)
                        {
        ...
        /// <param name="fs"></param>
        /// <returns></returns>
        public static DataTable ExcelToTableForXLS(FileStream fs)
        {
            DataTable dt = new DataTable();
