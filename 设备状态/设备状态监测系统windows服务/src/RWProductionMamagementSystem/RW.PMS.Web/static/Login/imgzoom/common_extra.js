function _relatedlinks(rlinkmsgid) {
        ...
        for (var j = 0; j < relatedlink.length; j++) {
            if (relatedlink[j] && !relatedid[j]) {
                if (relatedlink[j]['surl'] != '') {
                    var ra = '<a href="' + relatedlink[j]['surl'] + '" target="_blank" class="relatedlink">' +
    ...
    (function ($) {
        $('#' + rlinkmsgid + ' [initialized=true]').each(function () {
...
function _checksec(type, idhash, showmsg, recall, modid) {
    ...
    x.get('misc.php?mod=sec' + type + '&action=check&inajax=1&modid=' + modid + '&idhash=' + idhash + '&secverify=' + (
        BROWSER.ie && document.charset == 'utf-8' ? encodeURIComponent(secverify) : secverify), function (s) {
...
function _showdistrict(container, elems, totallevel, changelevel, containertype) {
    ...
    var url = "home.php?mod=misc&ac=ajax&op=district&container=" + container + "&containertype=" + containertype +
        "&province=" + elems[0] + "&city=" + elems[1] + "&district=" + elems[2] + "&community=" + elems[3] + "&pid=" +
        pid + "&cid=" + cid + "&did=" + did + "&coid=" + coid + '&level=' + totallevel + '&handlekey=' + container +
        '&inajax=1' + (!changelevel ? '&showdefault=1' : '');
    ...
    var showselect_row = function (inpid, s, v, notime, rettype) {
        if (v >= 0) {
            if (!rettype) {
                ...
                return '<a href="javascript:;" onclick="$(\'' + inpid + '\').value = \'' + d.getFullYear() + '-' +
                    month + '-' + day + (!notime ? ' ' + hour + ':' + minute : '') + '\'">' + s + '</a>';
            } else {
        ...
        if (!t) {
            ...
            if ($(t)) {
                ...
                for (i = 0; i < lis.length; i++) {
                    s += '<a href="javascript:;" onclick="$(\'' + inpid + '\').value = this.innerHTML;$(\'' + obj.id +
                        '_menu\').style.display=\'none\'">' + lis[i].innerHTML + '</a>';
                }
...
function _zoom(obj, zimg, nocover, pn, showexif) {
    ...
    var pn = !pn ? 0 : 1;
    var maxh = (document.documentElement.clientHeight ? document.documentElement.clientHeight : document.body.clientHeight) -
        70;
    ...
    var showloading = function (zimg, pn) {
        ...
        $(menuid + '_waiting').style.top = ((document.documentElement.clientHeight - 42) / 2 + Math.max(document.documentElement
    ...
    if (!menu && !pn) {
        ...
        if (cover) {
            //添加表头按钮
            menu.innerHTML = '<div class="zoominner" id="' + menuid +
                '_zoomlayer" style="display:none">'+
                '<p>'+
                '<span class="y">'+
                ...
                '</span>鼠标滚轮缩放图片'+
                '</p>' +
                '<div class="zimg_p" id="' + menuid + '_picpage"></div><div class="hm" id="' + menuid +
                '_img"></div></div>';
        } else {
            menu.innerHTML = '<div class="popupmenu_popup" id="' + menuid +
                ...
                menuid + '_picpage"></div><div class="hm" id="' + menuid + '_img"></div></div>';
        }
    ...
    if (typeof zoomgroup == 'object' && zoomgroup[obj.id] && typeof aimgcount == 'object' && aimgcount[zoomgroup[obj.id]]) {
        ...
        if (authorlength > 1) {
            ...
            if (authorcurrent !== '') {
                ...
                picpage +=
                    ' <div id="zimg_prev" onmouseover="dragMenuDisabled=true;this.style.backgroundPosition=\'0 50px\'" onmouseout="dragMenuDisabled=false;this.style.backgroundPosition=\'0 -100px\';" onclick="_zoom_page(\'' +
                    paid + '\', ' + (showexif ? 1 : 0) + ')" class="zimg_prev"><strong>上一张</strong></div> ';
                paid = authorcurrent < authorlength - 1 ? authorimgs[authorcurrent + 1] : authorimgs[0];
                picpage +=
                    ' <div id="zimg_next" onmouseover="dragMenuDisabled=true;this.style.backgroundPosition=\'100% 50px\'" onmouseout="dragMenuDisabled=false;this.style.backgroundPosition=\'100% -100px\';" onclick="_zoom_page(\'' +
                    paid + '\', ' + (showexif ? 1 : 0) + ')" class="zimg_next"><strong>下一张</strong></div> ';
            }
            if (picpage) {
...
function _zoom_page(paid, showexif) {
    var imagesrc = $('aimg_' + paid).getAttribute('zoomfile') ? $('aimg_' + paid).getAttribute('zoomfile') : $('aimg_' +
        paid).getAttribute('file');
    ...
    for (var i = 0; i < len; i++) {
        ...
        if (hasClass(arrTab[i], 'frame') || hasClass(arrTab[i], 'tab')) {
            ...
            for (var j in arrTab[i].childNodes) {
                if (typeof arrTab[i].childNodes[j] == 'object' && !hasClass(arrTab[i].childNodes[j], 'title')) arrColumn
                    .push(arrTab[i].childNodes[j]);
            }
            var frameContent = document.createElement('div');
            frameContent.id = tabId + '_content';
            frameContent.className = hasClass(arrTab[i], 'frame') ? 'content cl ' + arrTab[i].className.substr(arrTab[i]
                .className.lastIndexOf(' ') + 1) : 'content cl';
        ...
        li.innerHTML = '<a href="' + href + '"' + onclick + ' onfocus="this.blur();" ' + style + '>' + imgs + li.innerHTML +
            '</a>';
...
function slideshow(el) {
    ...
    if (!this.slidebar) {
        if (!this.slidenum && !this.slidestep) {
            ...
            for (var i = 0; i < this.length; i++) {
                html += '<li on' + this.barmevent + '="slideshow.entities[' + this.id + '].xactive(' + i +
                    '); return false;">' + (i + 1).toString() + '</li>';
            }
            html += '</ul>';
    ...
    this.runRoll = function () {
        for (var i = 0; i < this.slidenum; i++) {
            if (this.slideshows[i] && typeof this.slideshows[i].style != 'undefined') this.slideshows[i].style.display =
                'block';
...
function slidexactive(step) {
    ...
    currentslideele = hasClass(aim, 'slidebarup') || hasClass(aim, 'slidebardown') || hasClass(parent, 'slidebar') ?
        aim : null;
...
function _showPrompt(ctrlid, evt, msg, timeout, classname) {
    ...
    if (ctrlid) {
        ...
        msg =
            '<table cellspacing="0" cellpadding="0" class="popupcredit"><tr><td class="pc_l">&nbsp;</td><td class="pc_c"><div class="pc_inner">' +
...
function creditShow(creditinfo, notice, basev, bk, first, creditrule) {
    ...
    for (i = 1; i <= 8; i++) {
        ...
        if (notice[i] !== '0' && creditinfo[i]) {
            s += '<span>' + creditinfo[i][0] + (notice[i] != 0 ? (notice[i] > 0 ? '<em>+' : '<em class="desc">') +
                notice[i] + '</em>' : '') + creditinfo[i][1] + '</span>';
        }
        if (notice[i] > 0) {
    ...
    if (!$('creditpromptdiv')) {
        showPrompt(null, null, '<div id="creditpromptdiv">' + (creditrule ? '<i>' + creditrule + '</i> ' : '') + s +
            '</div>', 0);
    } else {
...
function _showColorBox(ctrlid, layer, k, bgcolor) {
    ...
    if (!$(ctrlid + '_menu')) {
        ...
        var coloroptions = ['Black', 'Sienna', 'DarkOliveGreen', 'DarkGreen', 'DarkSlateBlue', 'Navy', 'Indigo',
            ...
            'Magenta', 'Orange', 'Yellow', 'Lime', 'Cyan', 'DeepSkyBlue', 'DarkOrchid', 'Silver', 'Pink', 'Wheat',
            'LemonChiffon', 'PaleGreen', 'PaleTurquoise', 'LightBlue', 'Plum', 'White'];
        var colortexts = ['黑色', '赭色', '暗橄榄绿色', '暗绿色', '暗灰蓝色', '海军色', '靛青色', '墨绿色', '暗红色', '暗桔黄色', '橄榄色', '绿色', '水鸭色',
            '蓝色', '灰石色', '暗灰色', '红色', '沙褐色', '黄绿色', '海绿色', '间绿宝石', '皇家蓝', '紫色', '灰色', '红紫色', '橙色', '黄色', '酸橙色',
            '青色', '深天蓝色', '暗紫色', '银色', '粉色', '浅黄色', '柠檬绸色', '苍绿色', '苍宝石绿', '亮蓝色', '洋李色', '白色'];
        var str = '';
        for (var i = 0; i < 40; i++) {
            str += '<input type="button" style="background-color: ' + coloroptions[i] + '"' + (typeof setEditorTip ==
                    'function' ? ' onmouseover="setEditorTip(\'' + colortexts[i] +
                    '\')" onmouseout="setEditorTip(\'\')"' : '') + ' onclick="' + (typeof wysiwyg == 'undefined' ?
                    'seditor_insertunit(\'' + k + '\', \'[' + tag1 + '=' + coloroptions[i] + ']\', \'[/' + tag1 +
                    ']\')' : (ctrlid == editorid + '_tbl_param_4' ? '$(\'' + ctrlid + '\').value=\'' + coloroptions[i] +
                        '\';hideMenu(2)' : 'discuzcode(\'' + tag2 + '\', \'' + coloroptions[i] + '\')')) + '" title="' +
                colortexts[i] + '" />' + (i < 39 && (i + 1) % 8 == 0 ? '<br />' : '');
        }
        menu.innerHTML = str;
        $('append_parent').appendChild(menu);
    }
...
function _toggle_collapse(objname, noimg, complex, lang) {
    ...
    if (!noimg) {
        ...
        if (img.tagName != 'IMG') {
            ...
            img.src = img.src.indexOf('_yes.gif') == -1 ? img.src.replace(/_no\.gif/, '_yes\.gif') : img.src.replace(
                /_yes\.gif/, '_no\.gif');
        }
        img.blur();
    }
...
function _showCreditmenu() {
    if (!$('extcreditmenu_menu')) {
        ...
        menu.innerHTML = '<div class="p_opt"><img src="' + IMGDIR +
...
function _imageRotate(imgid, direct) {
    ...
    if (BROWSER.ie) {
        if (!isNaN(image.getAttribute('om'))) {
            image.style.marginBottom = (image.getAttribute('om') + (BROWSER.ie < 8 ? 0 : (deg == 90 || deg == 270 ?
                Math.abs(ow - oh) : 0))) + 'px';
        }
...
function _createPalette(colorid, id, func) {
    var iframe = "<iframe name=\"c" + colorid +
    ...
    window.frames["c" + colorid + "_frame"].location.href = url + "image/admincp/getcolor.htm?c" + colorid + "|" + id +
        func;
    showMenu({
        'ctrlid': 'c' + colorid
    });
...
function _setShortcut() {
    $('shortcuttip').onclick = function () {
        ...
        msg +=
            'onclick="this.href = \'forum.php?mod=misc&action=shortcut\';this.click();saveUserdata(\'setshortcut\', 1);"';
