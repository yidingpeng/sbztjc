...
function checkPwdComplexity(firstObj, secondObj, modify) {
    ...
    firstObj.onkeyup = function () {
        if (pwlength == 0 || $(firstObj.id).value.length >= pwlength) {
            ...
            errormessage(firstObj.id, '<span class="passlevel passlevel' + passlevel + '">密码强度:' + passlevels[
                passlevel] + '</span>');
        }
    };
...
var emailMenuST = null,
    emailMenui = 0,
    emaildomains = ['qq.com', '163.com', 'sina.com', 'sohu.com', 'yahoo.com', 'gmail.com', 'hotmail.com'];
...
function emailMenu(e, id) {
    ...
    for (var i = 0; i < emaildomains.length; i++) {
        s += '<li><a href="javascript:;" onmouseover="emailMenuOp(5)" ' + (emailMenui == i ? 'class="a" ' : '') +
...
function checkusername(id) {
    ...
    if (username.match(/<|"/ig)) {
        errormessage(id, '账户名包含敏感字符');
        return;
    }
    var unlen = username.replace(/[^\x00-\xff]/g, "**").length;
    if (unlen < 3 || unlen > 15) {
        errormessage(id, unlen < 3 ? '账户名不得小于 3 个字符' : '账户名不得超过 15 个字符');
        return;
    }
    //var x = new Ajax();
    $('tip_' + id).parentNode.className = $('tip_' + id).parentNode.className.replace(/ p_right/, '');
    ...
    //jq.ajax({
    //    url: "/Account/VerifyUser?user=" + username,
    //    type: "GET",
    //    //data: { "user": username },
    //    async: false,
    //    //dataType: "json",
    //    success: function (data) {
    //        //errormessage(id, data);
    //        console.log("验证");
    //    },
    ...
    //x.get('forum.php?mod=ajax&inajax=yes&infloat=register&handlekey=register&ajaxmenu=1&action=checkusername&username=' +
    //    (BROWSER.ie && document.charset == 'utf-8' ? encodeURIComponent(username) : username.replace(/%/g, '%25').replace(
    //        /#/g, '%23')),
    //    function (s) {
    //        errormessage(id, s);
    //    });
...
function checkemail(id) {
    ...
    var x = new Ajax();
    $('tip_' + id).parentNode.className = $('tip_' + id).parentNode.className.replace(/ p_right/, '');
    x.get('forum.php?mod=ajax&inajax=yes&infloat=register&handlekey=register&ajaxmenu=1&action=checkemail&email=' +
        email,
        function (s) {
            errormessage(id, s);
        });
