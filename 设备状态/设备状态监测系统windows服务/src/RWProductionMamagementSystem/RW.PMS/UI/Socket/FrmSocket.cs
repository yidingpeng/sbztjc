...
namespace RW.PMS.WinForm.Sockets
{
    ...
    /// Socket测试
    /// </summary>
    public partial class FrmSocket : FormSkin
    {
        #region 构造函数
        public FrmSocket()
        {
        ...
        private void axTcpClient_OnStateInfo(string msg, SocketHelper.SocketState state)
        {
            ControlHelper.Invoke(this, delegate
            {
                ...
                txtLog.Text = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + " 状态[" + state.ToString() + "]消息[" + msg + "]\r\n" + txtLog.Text;
                if (msg == "已连接服务器")
                {
                    //连接控制器
                    //axTcpClient.SendCommand(Encoding.ASCII.GetBytes("00200001003         011"));
                    //axTcpClient.SendCommand(Encoding.ASCII.GetBytes("002000010050           "));
        ...
        private void tm_Tick(object sender, EventArgs e)
        {
            try
            {
                ...
                //if (this.comboBox2.SelectedItem.ToString() == "4N.m扳手")
                //{
                //    SendData("00200043000000000000\0");//保持通讯
                //}
                //else
                //{
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                axTcpClient.ServerIp = textBox1.Text.Trim();
                axTcpClient.ServerPort = Convert.ToInt32(textBox2.Text.Trim());
