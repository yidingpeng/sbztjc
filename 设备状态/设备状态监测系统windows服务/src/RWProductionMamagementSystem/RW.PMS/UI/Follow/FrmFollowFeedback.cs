...
namespace RW.PMS.WinForm.UI.Follow
{
    ...
    /// 信息反馈窗体
    /// </summary>
    public partial class FrmFollowFeedback : Form
    {
        ...
        private void FrmFollowFeedback_Load(object sender, EventArgs e)
        {
            bindData();
        }
        ...
        {
            List<entity.FollowFeedbackModel> list = BLL.GetFollowfeedback(Convert.ToInt32(frm_areaID), Convert.ToInt32(frm_gwID), Convert.ToInt32(frm_empId));
        ...
        {
            FrmFeedbackEdit feedback = new FrmFeedbackEdit(EDAEnums.ActionType.Add.ToString(),0);
        ...
        private void toolbtnEdit_Click(object sender, EventArgs e)
        {
            ...
            if (ID!=0)
            {
                FrmFeedbackEdit feedback = new FrmFeedbackEdit(EDAEnums.ActionType.Edit.ToString(), ID);
                DialogResult dialogresult = feedback.ShowDialog();
                //判断是否返回为OK，刷新窗体
                if (dialogresult == DialogResult.OK)
                {
        ...
        private void toolbtnDel_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (value == DialogResult.Yes)
                {
                    ...
                    if (ID != 0)
                    {
                        bool result = BLL.DeleteFollow_Feedback(ID);
                        if (result)
                        {
