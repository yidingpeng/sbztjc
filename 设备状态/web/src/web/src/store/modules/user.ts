...
import { getUserInfo, login, logout, socialLogin, ssologin } from '@/api/user'
import { getToken, removeToken, setToken } from '@/utils/token'
import { resetRouter } from '@/router'
import { isArray, isString } from '@/utils/validate'
import { tokenName } from '@/config'
import { gp } from '@gp'
import { getImageBase64 } from '@/api/system/upload'
...
export const useUserStore = defineStore('user', {
  ...
  actions: {
     ...
     * @param {*} userInfo
     */
    async ssologin(userInfo: any) {
      const {
        data: { [tokenName]: token },
      } = await ssologin(userInfo)
      this.afterLogin(token, tokenName)
    },
    /**
     * @description 第三方登录
    ...
    async getUserInfo() {
      ...
      if (
        ...
        if (avatar) {
          const data = await getImageBase64(avatar)
          //console.log(data.toString())
          this.setAvatar(data.toString())
        }
