<template>
  <el-table v-loading="state.loading" :data="state.treeList" default-expand-all row-key="id">
    <el-table-column type width="50">
      <template #default="scope">
        <el-checkbox v-model="state.list[scope.$index].selfChecked" :indeterminate="state.list[scope.$index].isHalf"
          @change="(value) => handleCheckAll(value, scope.$index)" />
      </template>
    </el-table-column>
    <el-table-column label="菜单" prop="title" show-overflow-tooltip width="200px" />
    <el-table-column label="操作">
      <template #default="scope">
        <el-checkbox-group v-model="state.list[scope.$index].opChecked"
          @change="(value) => handleOpCheck(value, scope.$index)">
          <el-checkbox v-for="(item, key) in scope.row.operation" :key="key" :disabled="setChkStatue(item.id, scope.row)"
            :label="item.id">
            {{ item.title }}
          </el-checkbox>
        </el-checkbox-group>
